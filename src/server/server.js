...
app.post('/getMovies', async (req, res) => {
    ...
    const movies = await getMovies(inputs.eventStart, inputs.eventEnd, inputs.yearStart, inputs.yearEnd)
    res.send(movies);
})
...
async function getMovies(firstId, lastId, firstYear, lastYear) {
    ...
    for (let index = firstId; index <= lastId; index++) {
        for (let year = firstYear; year <= lastYear; year++) {
            fetchYears.push(fetchYear(browser, EVENTS[Object.keys(EVENTS)[index]], year));
        }
